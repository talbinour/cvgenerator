...
const ParentComponent = () => {
  const navigate = useNavigate();
    ...
    const saveCVToServer = async () => {
      try {
        ...
        navigate(`/model6-user/${userId}/${cvId}/${id}`);
      } catch (error) {
  ...
  const removeInterest = (index) => {
    const newInterests = [...cvModel.interests];
    newInterests.splice(index, 1);
    setCvModel({ ...cvModel, interests: newInterests });
  };
  return (
            ...
            <div className={`${styles.contactInfo} ${styles.education}`}>
             ...
             <ul>
                ...
                <li key={index}>
                  ...
                  />
                      <i style={{ cursor: 'pointer', color: '#980110 ', margin: '5px', padding: '1px 2px', border: '1px ', borderRadius: '5px'  }} className="fas fa-trash-alt" onClick={() => removeEducation(index)}></i>
                </li>
              ))}
              ...
              <ul>
                {cvModel.languages.map((lang, index) => (
                  <div className={styles.box} key={index}>
                    ...
                    <i style={{ cursor: 'pointer', color: '#980110',margin: '5px', padding: '1px 2px', border: '1px ', borderRadius: '5px'  }} className="fas fa-trash-alt" onClick={() => removeLanguage(index)}></i>
                  </div>                  
                ))}
              </ul>
            </div>
            ...
            <div className={styles.about}>
              ...
              {cvModel.experiences.map((exp, index) => (
                  <div className={styles.box} key={index}>
                    ...
                    <i style={{ cursor: 'pointer', color: '#980110', margin: '5px', padding: '1px 2px', border: '1px ', borderRadius: '5px'  }} className="fas fa-trash-alt"onClick={() => removeExperience(index)}></i>
                  </div>
                ))}
                <div className={styles.buttonContainer}>
              ...
              <h2 className={styles.title2}>COMPÉTENCES PROFESSIONNELLES</h2>
                {cvModel.professionalSkills.map((skill, index) => (
                  <div className={styles.box} key={index}>
                    ...
                    <i style={{ cursor: 'pointer', color: '#980110', margin: '5px', padding: '1px 2px', border: '1px ', borderRadius: '5px' }}  className="fas fa-trash-alt" onClick={() => removeSkill(index)}></i>
                  </div>
                ))}
              <div className={styles.buttonContainer}>
              ...
              <ul contentEditable={true}>
                {cvModel.interests.map((interest, index) => (
                  <li key={index}><i className="fa fa-circle" aria-hidden="true"></i>{interest}
                      <i style={{ cursor: 'pointer', color: '#980110', margin: '5px', padding: '1px 2px', border: '1px ', borderRadius: '5px' }} className="fas fa-trash-alt" onClick={() => removeInterest(index)}></i>
                  </li>
